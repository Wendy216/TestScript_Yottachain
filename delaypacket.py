...
def usage():
    print "usage: python delaypacket.py [report_file] eth0/docker0"
    sys.exit(1)

if __name__ == '__main__':
    if len(sys.argv) == 3:
        report_file = sys.argv[1]
        fp.set_delay(sys.argv[2])
    elif len(sys.argv) == 2:
        fp.set_delay(sys.argv[1])
    else:
        usage()
    ...
    for config in list_config:
        if "config_dir" in config["configfilename"]:
            ...
            logger.info("\nBegin to run delay packet.")
            fp.append_report(report_file, "\n\n[%s] - begin to run test delay packet.\n"
